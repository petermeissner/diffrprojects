...
## 
## $project_id
## [1] "f4d22aed08560f5a81abe39de426c9f9"
```
...
If no arguments are specified text_link will linkthe first text to the second, the third to th fourth, the fourth to the fifths and so on.
...
To get an idea of what links are currently specified we can directly access the link data field or/and ask R to transforme the list found there into a data.frame. 
...
In this example we are searching for the word *"it"* in text 1 an code each instance. 
...
To show this feature we use the text_inherit method and we will start with a fresh example. A new project with two texts. The first text gets some codings, then they are aligned in a last step codings are transfered from from one text to the other via the text_data_inherit method. 
...
Diffrprojects also allow for storing and loading projaect to and from disk. 
...
Diffrprojects has two other packages it relies heavily on and one package that add further features. 
